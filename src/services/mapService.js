    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        ...
        (error) => {
          ...
          // Default to a central location without logging an error
          const defaultLocation = [20.5937, 78.9629]; // central India
      ...
      resolve([20.5937, 78.9629]); // central India
    }
  });
};
...
export const watchPosition = (callback) => {
  ...
  const watchId = navigator.geolocation.watchPosition(
    ...
    (error) => {
      ...
      console.warn('Geolocation watching unavailable:', error.message);
      
      // You could optionally call the callback with a default location here
